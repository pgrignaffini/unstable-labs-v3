...
function FileReviews({ paperId }: Props) {
    ...
    return (
        <div className="bg-transparent text-acid p-5 border overflow-x-hidden">
            ...
            <div className="h-56 overflow-y-scroll scrollbar-hide">
                {isLoadingPaperReviews ?
                    <p className="text-acid text-center text-md">Loading reviews...</p>
                    : paperReviews?.length ? paperReviews?.map(review => (
                        <div key={review?.id} className="flex items-center space-x-2 mb-3">
                            ...
                            <p className="text-sm flex-1">
                                <span className="font-bold text-slate-500">{review?.user?.name}{": "}</span>
                                {review?.text}
                            </p>
                            <div className="flex items-center space-x-2">
                                <TimeAgo className="text-slate-300 text-sm px-4" date={review?.createdAt} />
                            </div>
                        </div>
                    )) : <p className="text-acid text-center text-md">Be the first to post a review!</p>}
            </div>
            {session ?
                <form className="flex items-center p-4 border-t" onSubmit={sendReview}>
                    <input
                        ...
                        className="border-none bg-transparent text-white flex-1 focus:ring-0 outline-none"
                    />
