...
function MintVialButton({ index, numberOfVials, style }: Props) {
  ...
  const txValue = vialPrice
    ?.mul(BigNumber.from(numberOfVials))
    ?.add(BigNumber.from(500000000000000));

  const tokenURI =
    style === "remix"
      ? RemixMetadataURL
      : style === "concept"
      ? `${ConceptMetadataURL}/${index}.json`
      : style === "collection"
      ? `${CollectionMetadataURL}/${index}.json`
      : style === "freestyle"
      ? FreestyleMetadataURL
      : "";
  ...
  const { config } = usePrepareContractWrite({
    ...
    args: [
      ...
      numberOfVials,
      {
        value: vialPrice?.mul(BigNumber.from(numberOfVials)),
  ...
  const notEnoughBalanceMessage =
    "You don't have enough balance to mint this vial and pay for tx fees";

  return (
    <div className="py-4">
      <div className="flex flex-col items-center justify-evenly space-y-4 pt-3">
        <SolidButton
          type="submit"
          color="green"
          error={!isConnected}
          onClick={() => {
            if (balance?.value && txValue && balance?.value.lt(txValue)) {
              toast.custom((t) => (
                <Toast toastInfo={t} message={notEnoughBalanceMessage} error />
              ));
              return;
            }
            setIsLoading(true);
      ...
      {isMinting && (
        <div className="mt-4">
          <TxHash className="text-black" hash={`${vialData?.hash}`} />
        </div>
      )}
    </div>
  );
